...
void BasicSc2Bot::OnGameStart() {
  ...
  build_order.push(BuildOrderItem(
      17, UNIT_TYPEID::ZERG_SPAWNINGPOOL)); // At 17 cap, build a Spawning Pool
  build_order.push(BuildOrderItem(
  ...
  if (buildItem.is_unit) {
    ...
    case UNIT_TYPEID::ZERG_HATCHERY: {
      ...
      if (drone && observation->GetMinerals() >= 300) {
        ...
        Point2D build_position =
            findBuildPositionNear(nearest_new_mineral_loc->pos);
    ...
    case UNIT_TYPEID::NEUTRAL_VESPENEGEYSER: {
      ...
      if (drone) {
        ...
        Point2D build_position =
            Point2D(nearest_new_mineral_loc->pos.x + rx * 15.0f,
                    nearest_new_mineral_loc->pos.y + ry * 15.0f);
