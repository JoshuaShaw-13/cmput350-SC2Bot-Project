...
void BasicSc2Bot::OnStep() {
    ...
    if (state.enemyBaseLocations.empty() && !attacking_roaches.empty()) {
        std::cout
            << "No enemy buildings detected. Starting scouting with roaches."
            << std::endl;
        ...
        for (const auto &roach : scouting_roaches) {
            if (!unscouted_mineral_patches.empty()) {
                ...
                Actions()->UnitCommand(roach, ABILITY_ID::MOVE_MOVE,
                                       target_patch);
        ...
        if (!roach_scouting_assignments.empty()) {
            std::cout
                << "Enemy building found! Canceling scouting and attacking."
                << std::endl;
...
void BasicSc2Bot::OnUnitIdle(const Unit *unit) {
    ...
    void BasicSc2Bot::OnUnitDestroyed(const Unit *unit) {
        if (unit->unit_type == UNIT_TYPEID::ZERG_DRONE &&
            ...
            gas_harvesting_drones.erase(unit->tag);
                   ...
                   unit->alliance == sc2::Unit::Alliance::Self) {
            build_order.push_front(
                BuildOrderItem(0, UNIT_TYPEID::ZERG_OVERLORD));
            ...
            if (is_structure) {
                ...
                if (it != state.enemyBaseLocations.end()) {
                    state.enemyBaseLocations.erase(
                        it, state.enemyBaseLocations.end());
                    std::cout
                        << "Enemy building destroyed! Removed from building "
                           "locations."
                        << std::endl;
                }
            }
    ...
    void BasicSc2Bot::OnUnitCreated(const Unit *unit) {
        ...
        case UNIT_TYPEID::ZERG_ROACH: {
            ...
            std::cout
                << "Roach created and added to current group. Group size: "
        ...
        case sc2::UNIT_TYPEID::ZERG_ZERGLING: {
            ...
            if (added_scout) {
                ...
                scouts.insert(
                    std::pair<Tag, Scout>(unit->tag, Scout(unit->tag, index)));
    ...
    const Point2D BasicSc2Bot::getValidRallyPoint(
        const Point2D &base_position, QueryInterface *query, float max_radius,
        float step) {
        ...
        for (float radius = step; radius <= max_radius; radius += step) {
            for (float angle = 0.0f; angle < 360.0f;
                ...
                if (query->Placement(ABILITY_ID::RALLY_UNITS,
                                     potential_point)) {
    ...
    const Unit *BasicSc2Bot::FindNearestMineralPatchForHarvest(
        const Point2D &start) {
        Units units = Observation()->GetUnits(Unit::Alliance::Neutral);
        float distance = std::numeric_limits<float>::max();
        const Unit *target = nullptr;
    ...
    Point2D BasicSc2Bot::findBuildPositionNearMineral(
        const Point2D &target_position) {
        const float hatchery_size = 5.0f; // Hatchery size in grid squares
        const float build_radius =
            hatchery_size / 2.0f; // Half the size for radius
    ...
    Point2D BasicSc2Bot::findBuildPosition(const Point2D &start) {
        ...
        for (float x = -search_radius; x <= search_radius;
             x += search_increment) {
    ...
    bool BasicSc2Bot::tryBuild(struct BuildOrderItem buildItem) {
        ...
        if (buildItem.ability == ABILITY_ID::MORPH_LAIR) {
            ...
            if (initial_hatchery && initial_hatchery->build_progress == 1.0f &&
                ...
                if (observation->GetMinerals() >= 150 &&
                    ...
                    std::cout << "Upgrading initial hatchery to Lair."
                              << std::endl;
                    return true;
                } else {
        ...
        if (buildItem.is_unit) {
            ...
            case UNIT_TYPEID::ZERG_ROACH: {
                ...
                const Units roach_warrens = observation->GetUnits(
                    Unit::Alliance::Self,
                    IsUnit(UNIT_TYPEID::ZERG_ROACHWARREN));
            ...
            case UNIT_TYPEID::ZERG_ZERGLING: {
                const Units spawning_pools = observation->GetUnits(
                    Unit::Alliance::Self,
                    IsUnit(UNIT_TYPEID::ZERG_SPAWNINGPOOL));
            ...
            case UNIT_TYPEID::ZERG_HATCHERY: {
                ...
                if (drone && observation->GetMinerals() >= 300) {
                    ...
                    std::cout
                        << "calculated hatchery point: " << hatchery_location.x
                    ...
                    if (build_position.x != 0.0f || build_position.y != 0.0f) {
                        Actions()->UnitCommand(
                            drone, ABILITY_ID::BUILD_HATCHERY, build_position);
            ...
            case UNIT_TYPEID::ZERG_SPAWNINGPOOL: {
                ...
                if (drone && observation->GetMinerals() >= 200) {
                    ...
                    Actions()->UnitCommand(
                        drone, ABILITY_ID::BUILD_SPAWNINGPOOL, build_position);
                    return true;
                }
                break;
            }
        ...
        if (buildItem.ability != ABILITY_ID::INVALID && !buildItem.is_unit) {
            Units structures = observation->GetUnits(
                Unit::Alliance::Self, IsUnit(buildItem.unit_type));
            if (!structures.empty()) {
    ...
    const Unit *BasicSc2Bot::findNextNearestMineralGroup(
        const Unit *mineral_loc_a) {
        /**
    ...
    double BasicSc2Bot::getVectorDifferenceMagnitude(Point2D vec_a,
                                                     Point2D vec_b) {
        /**
    ...
    Point2D BasicSc2Bot::FindPlacementLocation(AbilityID ability,
        ...
        for (int i = 0; i < 20;
             ++i) { // Increased iterations for better chances
            float rx =
                GetRandomScalar() * 10.0f - 5.0f; // Center around near_point
    ...
    void BasicSc2Bot::BalanceWorkers() {
        ...
        for (const auto &overBase : overSaturatedBases) {
            ...
            Units drones = observation->GetUnits(
                Unit::Alliance::Self, IsUnit(UNIT_TYPEID::ZERG_DRONE));
            ...
            for (const auto &drone : drones) {
                ...
                if (target->unit_type == UNIT_TYPEID::NEUTRAL_MINERALFIELD &&
                    ...
                    dronesToTransfer.push_back(
                        drone); // If it's gathering minerals and near the
                                // hatchery, transfer it
            ...
            for (const auto &drone : dronesToTransfer) {
                ...
                if (underBase) {
                    ...
                    if (mineral_patch) {
                        Actions()->UnitCommand(
                            drone, ABILITY_ID::HARVEST_GATHER, mineral_patch);
